...
- Keep `targets.json` fields compatible with workflow consumers, including `target`, `repo`, `ref`, and `artifacts_release_tag`. 种子配置目录名恒等于 `target`（`openwrt-configs/immortalwrt/<target>/`），不存在独立的 `config` 字段。包格式/签名机制不再由 target 静态配置（无 `apk_signing` 字段），由构建时从 `.config`/SDK `Config-build.in` 自动探测 `CONFIG_USE_APK`。
