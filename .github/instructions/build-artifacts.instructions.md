...
- Publish SDK and ImageBuilder archives for each target under the shared `artifacts-<target>` release tag. Maintain separate `sdk-index.json` and `ib-index.json` assets so the two indexes cannot overwrite each other.
