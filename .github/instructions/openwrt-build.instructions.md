...
- Do not export `CC`/`CXX` wrappers at the workflow level at all: seeds set `CONFIG_CCACHE=y`, so `rules.mk` auto-wraps `TARGET_CC:=ccache_cc` and `HOSTCC:=ccache $(HOSTCC)`. Environment `CC`/`CXX` exports are redundant and can leak the system `gcc` into packages that do not read `TARGET_CONFIGURE_OPTS`. The `no-cache` strategy only skips `actions/cache` persistence; in-build ccache still follows `CONFIG_CCACHE`.
