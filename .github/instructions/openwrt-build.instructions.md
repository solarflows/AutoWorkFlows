...
- The SDK incremental path owns the rolling `immwrt-v2-sdk-hostpkg-<target>-<run_id>` and `immwrt-v2-sdk-ccache-<target>-<run_id>` snapshots. It may restore and save its own compiler ccache snapshot, but must never save to or purge the full-build `immwrt-v2-ccache-<target>-*` namespace. The full executor remains the owner of shared ccache cleanup; `source_sha` is artifact/SDK-IB matching metadata, not part of the SDK hostpkg key.
