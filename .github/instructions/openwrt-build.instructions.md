...
- Feed patches under `.github/upstream-sync/patches/packages/` (applied by `upstream-sync.yml` via `git apply`) must be validated with `git apply --check` before merging. Every content line inside a hunk must start with `+`, `-`, or a space; a missing `+` prefix on an added line makes `git apply` report `corrupt patch`. `upstream-sync.yml` is also triggered by `push.paths` on these patches — pushing a patch change re-runs the sync automatically.
