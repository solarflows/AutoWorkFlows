...
在 `OpenWRT_Packages_Updater.yml` 的全局补丁目录 `.github/custom/feeds/custom/patches/` 添加 `fix-shadowsocksr-libev-configure.patch`, 修改包 Makefile, 注册一个 `Hooks/Prepare/Post` hook:
...
- mt798x 同补丁走 openwrt-packages 链路: `.github/custom/feeds/custom/overwrite/mt798x/smartdns/patches/{100,101}` + `patches/mt798x/0009-smartdns-bump-48.4.patch` (该 target 的 smartdns 来自 `package/solarflows/smartdns` core 包, feeds 同名包不生效)。
